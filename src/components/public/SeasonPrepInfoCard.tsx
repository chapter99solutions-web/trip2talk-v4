...
import type { SeasonPrepCard } from '../../lib/seasonPrepGuide';

type Props = {
  card: SeasonPrepCard;
};

export default function SeasonPrepInfoCard({ card }: Props) {
  const { lang } = useI18n();
  const isTh = lang === 'TH';

  return (
    <article className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 md:p-6 shadow-sm">
